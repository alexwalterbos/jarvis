...
import com.awalterbos.jarvis.hub.data.daos.Sayings;
import com.awalterbos.jarvis.hub.data.entities.Saying;
import com.awalterbos.jarvis.hub.resources.SayingsResource;
...
public class SayingsResourceTest {
	...
	@Before
	public void setup() {
		doReturn(saying).when(DAO).findByIdOrThrow(eq(1L));
		doReturn(saying).when(DAO).persistOrMerge(any(Saying.class));
		doThrow(new EntityNotFoundException()).when(DAO).findByIdOrThrow(2L);
